  ...
  <div class="container">
      ...
      <el-row>
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="职务">
            <el-input v-model="keyword.duty" size="mini" />
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
    ...
    >
      <div>
        <el-row>
          <el-col :span="24">
            <el-checkbox v-model="eye.duty">职务</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.name">联系人</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.tel">电话</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.email">电子邮件</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.phone">手机号码</el-checkbox>
          </el-col>
          <el-col :span="24">
    ...
    <component
      v-if="showViewPopup"
      :is="'CustomerContactViewPopup'"
      :id="currentClickRow.id"
      @close="showViewPopup = false"
    />
    <component
      v-if="showEditPopup"
      :is="'CustomerContactEditPopup'"
      :id="currentClickRow.id"
...
export default {
  ...
  components: {
    ...
    CustomerContactViewPopup: () => import('./customerContactViewPopup.vue'),
    CustomerContactEditPopup: () => import('./customerContactEditPopup.vue'),
    CustomerContactDeletePopup: () => import('./customerContactDeletePopup.vue')
  },
  filters: {
  },
  data() {
  ...
  methods: {
    async search() {
      ...
      let query = '*:*'
      if (Validate.isNotEmpty(keyword.duty)) {
        query += ` AND duty:${keyword.duty}`
      }
      if (Validate.isNotEmpty(keyword.name)) {
        query += ` AND name:${keyword.name}`
      }
      if (Validate.isNotEmpty(keyword.tel)) {
        query += ` AND tel:${keyword.tel}`
      }
      ...
      const res = await customerContactServer.search({
        query,
        page: this.page - 1,
        size: this.size
      })
