<template>
  <div>
    <el-dialog
      ...
      :title="id ? '提单放单-edit' : 'new-提单放单'"
      top="10vh"
      @close="close"
    >
      <div>
        <OrderNoticeEdit ref="orderNoticeEdit" :id="id" @loadingChange="loadingChange" />
      </div>
...
export default {
  ...
  methods: {
    ...
    async save() {
      const formData = await this.$refs.orderNoticeEdit.save()
