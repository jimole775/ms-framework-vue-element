<template>
  <div>
    ...
    <component v-if="showEdit" :is="'OperateUnitEditPopup'" :id="id" @close="showEdit = false" />
  </div>
</template>
...
export default {
  name: 'OperateUnitViewPopup',
  components: {
    OperateUnitEditPopup: () => import('./operateUnitEditPopup.vue')
  },
  filters: {
  },
  props: {
  ...
  methods: {
    ...
    async getData() {
      const res = await operateUnitServer.get(this.id)
