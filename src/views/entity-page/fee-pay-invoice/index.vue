  ...
  <div class="container">
    <el-form :inline="true" :model="keyword" class="demo-form-inline" @keyup.enter.native="commitSearch">
      <el-row>
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="应付金额(RMB)">
            <el-input v-model="keyword.amtRmb" size="mini" />
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
    ...
    >
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.amtRmb">应付金额(RMB)</el-checkbox>
          </el-col>
          <el-col :span="24">
    ...
    <component
      v-if="showViewPopup"
      :is="'FeePayInvoiceViewPopup'"
      :id="currentClickRow.id"
      @close="showViewPopup = false"
    />
    <component
      v-if="showEditPopup"
      :is="'FeePayInvoiceEditPopup'"
      :id="currentClickRow.id"
...
export default {
  name: 'FeePayInvoiceManagement',
  components: {
    ExportPopup: () => import('./../../../components/excel/ExportPopup.vue'),
    DepartmentSelect,
    DepartmentViewPopup: () => import('./../department/departmentViewPopup'),
    FeePayInvoiceViewPopup: () => import('./feePayInvoiceViewPopup.vue'),
    FeePayInvoiceEditPopup: () => import('./feePayInvoiceEditPopup.vue'),
    FeePayInvoiceDeletePopup: () => import('./feePayInvoiceDeletePopup.vue')
  },
  filters: {
  },
  data() {
  ...
  methods: {
    async search() {
      ...
      if (Validate.isNotEmpty(keyword.amtRmb)) {
        query += ` AND amtRmb:${keyword.amtRmb}`
      }
      ...
      const res = await feePayInvoiceServer.search({
        query,
        page: this.page - 1,
        size: this.size
      })
