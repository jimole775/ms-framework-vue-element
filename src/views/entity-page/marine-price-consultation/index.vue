      ...
      <el-row>
        ...
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="合同号">
            <el-input v-model="keyword.pi" size="mini" clearable/>
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="出货预告号">
            <el-input v-model="keyword.warnNo" size="mini" clearable/>
          </el-form-item>
        </el-col>
        <!-- <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          ...
          label="状态"
        >
          <template slot-scope="scope">{{ scope.row.status | MarinePriceConsultationStatusFilter }}</template>
        </el-table-column>
        <el-table-column
          v-if="eye.approver"
          show-overflow-tooltip
        ...
        >
          <template
            slot-scope="scope"
          >{{ scope.row.airRoute | MarinePriceConsultationAirRouteFilter }}</template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog :visible="showEye" width="360px" title="过滤" top="10vh" @close="showEye = false">
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.approver">当前审核人</el-checkbox>
          </el-col>
          <el-col :span="24">
      ...
      <template>
        ...
        >
          ...
          <el-table-column
            ...
            label="截关日期"
          >
            <template slot-scope="scope">{{ scope.row.expirationDate | parseTime('{y}-{m}-{d}') }}</template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
          ...
          >
            <template
              slot-scope="scope"
            >{{ scope.row.pricePeriodValidity | parseTime('{y}-{m}-{d}') }}</template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
            prop="totalAmount"
          ...
          >
            <template
              slot-scope="scope"
            >{{ scope.row.lastScheduledDate | parseTime('{y}-{m}-{d}') }}</template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
            prop="transportTime"
          ...
          <el-table-column show-overflow-tooltip prop="remark" sortable min-width="80" label="备注"/>
          <el-table-column
            show-overflow-tooltip
          ...
          >
            <template slot-scope="scope">
              <el-button
                type="text"
                size="mini"
                @click="openRecommendShippingCompanyViewPopup(scope.row.recommendShippingCompanyId)"
              >{{ scope.row.recommendShippingCompanyName }}</el-button>
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
          ...
          >
            <template slot-scope="scope">
              <el-button
                type="text"
                size="mini"
                @click="openShippingCompanyViewPopup(scope.row.shippingCompanyId)"
              >{{ scope.row.shippingCompanyName }}</el-button>
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
...
export default {
  ...
  components: {
    ...
    MarinePriceConsultationDeletePopup: () =>
      import('./marinePriceConsultationDeletePopup.vue')
  },
  filters: {
    MarinePriceConsultationStatusFilter:
      MarinePriceConsultationStatusEnumUtil.toTitleFilter,
    MarinePriceConsultationAirRouteFilter:
      MarinePriceConsultationAirRouteEnumUtil.toTitleFilter
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async getDetail(id) {
      const res = await marinePriceConsultationServer.get(id)
    ...
    handleRowDb(row) {
      this.$router.push({
        path: './marine-price-consultation/edit/' + row.enquirylistId
      })
    },
    clickBatchDeleteBtn() {
    ...
    clickExportBtn() {
      ...
      this.dataExport.forEach(item => {
        item.status = MarinePriceConsultationStatusEnumUtil.toTitleFilter(
          item.status
        )
        item.airRoute = MarinePriceConsultationAirRouteEnumUtil.toTitleFilter(
          item.airRoute
        )
