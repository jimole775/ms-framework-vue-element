...
export default {
  name: 'CreditCertificateDeletePopup',
  props: {
    list: {
      type: Array,
      default: null
  ...
  created() {
    ...
    this.waitDelete = this.waitDelete.filter(item => item.status === 1)
  },
  methods: {
    close() {
      this.$emit('close')
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          if (this.list != null) await creditCertificateServer.delete(this.list[0].lcBillId)
        } catch (e) {
