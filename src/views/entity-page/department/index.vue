      ...
      <el-row>
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="类型">
            <DepartmentTypeEnumSelect v-model="keyword.type" type="multiple" />            
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
      ...
      >
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            {{ scope.row.type | DepartmentTypeFilter }}
          </template>
        </el-table-column>
        <el-table-column
          v-if="eye.remark"
    ...
    >
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.thoseResponsible">负责人</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.type">类型</el-checkbox>
          </el-col>
          <el-col :span="24">
...
import DepartmentTypeEnumSelect from './../../../components/entity/department/DepartmentTypeEnumSelect'
import DepartmentTypeEnumUtil from './../../../components/enum/department/DepartmentTypeEnumUtil'
...
export default {
  ...
  components: {
    ...
    DepartmentTypeEnumSelect,
    DepartmentViewPopup: () => import('./departmentViewPopup.vue'),
    DepartmentEditPopup: () => import('./departmentEditPopup.vue'),
    DepartmentDeletePopup: () => import('./departmentDeletePopup.vue')
  },
  filters: {
    DepartmentTypeFilter: DepartmentTypeEnumUtil.toTitleFilter,
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    async search() {
      ...
      if (keyword.type && keyword.type.length > 0) {
        query += ' AND ('
        keyword.type.forEach((str, index) => {
          if (index > 0) {
            query += ' OR '
          }
          query += `type:${str}`
      ...
      const res = await departmentServer.search({
        query,
        page: this.page - 1,
        size: this.size
      })
