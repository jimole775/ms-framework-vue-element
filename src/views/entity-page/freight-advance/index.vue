<template>
  <div class="container">
      ...
      <el-row>
        ...
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="状态">
            <FreightAdvanceStatusEnumSelect v-model="keyword.status" type="multiple"/>
          </el-form-item>
        </el-col>
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="制单开始时间">
            <el-date-picker
              v-model="keyword.startCreateDate"
              size="mini"
              class="max-w"
              type="date"
              placeholder="选择开始时间"
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          ...
          label="经营单位"
        >
          <template slot-scope="scope">{{ scope.row.departmentId | OperaUnitFilter }}</template>
        </el-table-column>
        <!-- <el-table-column
          fixed="right"
          label="操作"
    ...
    <el-dialog :visible="showEye" width="360px" title="过滤" top="10vh" @close="showEye = false">
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.applyDate">创建时间</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.credentials">SAP凭证号</el-checkbox>
          </el-col>
          <el-col :span="24">
    ...
    <!-- <component
      v-if="showViewPopup"
      :is="'FreightAdvanceViewPopup'"
      :id="currentClickRow.id"
      @close="showViewPopup = false"
    />-->
    <!-- <component
      v-if="showEditPopup"
      :is="'FreightAdvanceEditPopup'"
      :id="currentClickRow.id"
...
import FreightAdvanceStatusEnumUtil from './../../../components/enum/freight-advance/FreightAdvanceStatusEnumUtil'
...
export default {
  ...
  components: {
    ...
    FreightAdvanceViewPopup: () => import('./freightAdvanceViewPopup.vue'),
    FreightAdvanceEditPopup: () => import('./freightAdvanceEditPopup.vue'),
    FreightAdvanceDeletePopup: () => import('./freightAdvanceDeletePopup.vue')
  },
  filters: {
    FreightStandardStatusFilter: FreightStandardStatusEnumUtil.toTitleFilter,
    FreightAdvanceStatusFilter: FreightAdvanceStatusEnumUtil.toTitleFilter,
  ...
  methods: {
    async search() {
      ...
      const res = await freightAdvanceServer.search({
        keyword,
        page: this.page - 1,
        size: this.size
      })
    ...
    clickEditBtn(row) {
      this.$router.push({
        path: './freight-advance/edit-sublist/' + row.ytId
      })
    },
    clickAddBtn() {
      this.$router.push({
        path: './freight-advance/edit-sublist/new'
      })
      this.reload()
    },
    clickBatchDeleteBtn() {
    ...
    clickExportBtn() {
      ...
      this.dataExport.forEach(item => {
        ...
        item.departmentId = OperaUnitEnumUtil.toTitleFilter(item.departmentId)
      })
      this.setExportData()
