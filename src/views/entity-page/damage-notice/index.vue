<template>
  <div class="container">
      ...
      <el-row>
        ...
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="状态">
            <!-- <DamageNoticeStatusEnumSelect v-model="keyword.status" type="multiple" /> -->
            <CommercialInvoiceStatusEnumSelect v-model="keyword.status" type="clearable"/>
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="创建时间">
            <el-date-picker v-model="keyword.createTime" size="mini" class="max-w" type="date"/>
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="买方名称">
            <el-input v-model="keyword.buyerName" size="mini"/>
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="可损原因">
            <DamageNoticeDamageReasonEnumSelect v-model="keyword.damageReason" type="multiple"/>
          </el-form-item>
        </el-col>
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          ...
          label="委托我司追讨"
        >
          <template slot-scope="scope">{{ scope.row.troverflag ? '是': '否' }}</template>
        </el-table-column>
        <el-table-column
          ...
          label="已办理贸易融资"
        >
          <template slot-scope="scope">{{ scope.row.financeflag ? '是': '否' }}</template>
        </el-table-column>
        <el-table-column
          ...
          label="已签署《索赔权转让协议》"
        >
          <template slot-scope="scope">{{ scope.row.ifinsuranturge ? '是': '否' }}</template>
        </el-table-column>
        <el-table-column
          v-if="eye.detainflag"
          show-overflow-tooltip
          ...
          label="是否已签署《赔款转让协议》"
        >
          <template slot-scope="scope">{{ scope.row.detainflag ? '是': '否' }}</template>
        </el-table-column>
        <el-table-column
          v-if="eye.ifbankurge"
          show-overflow-tooltip
          ...
          label="已向保兑银行催款"
        >
          <template slot-scope="scope">{{ scope.row.ifbankurge ? '是': '否' }}</template>
        </el-table-column>
        <el-table-column
          v-if="eye.ifarf"
          show-overflow-tooltip
          ...
          label="签署《应收账款转让协议》"
        >
          <template slot-scope="scope">{{ scope.row.ifarf ? '是': '否' }}</template>
        </el-table-column>
        <!-- <el-table-column
          v-if="eye.isDelete"
          show-overflow-tooltip
        ...
        >
          <!-- <template slot-scope="scope">
            <el-button type="text" size="mini" @click="openOperateUnitViewPopup(scope.row.entid)">{{ scope.row.operateUnit ? scope.row.operateUnit.id : '' }}</el-button>
          </template>-->
    ...
    <el-dialog :visible="showEye" width="360px" title="过滤" top="10vh" @close="showEye = false">
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.caseexplain">案情说明</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.measures">减损措施</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.troverflag">委托我司追讨</el-checkbox>
          </el-col>
          <el-col :span="24">
    ...
    <component
      v-if="showViewPopup"
      :is="'DamageNoticeViewPopup'"
      :id="currentClickRow.id"
      @close="showViewPopup = false"
    />
    <component
      v-if="showEditPopup"
      :is="'DamageNoticeEditPopup'"
      :id="currentClickRow.id"
...
export default {
  ...
  components: {
    ...
    DamageNoticeViewPopup: () => import('./damageNoticeViewPopup.vue'),
    DamageNoticeEditPopup: () => import('./damageNoticeEditPopup.vue'),
    DamageNoticeDeletePopup: () => import('./damageNoticeDeletePopup.vue')
  },
  filters: {
  ...
  methods: {
    ...
    clickExportBtn() {
      ...
      this.dataExport.forEach(item => {
        ...
        item.damageReason = DamageNoticeDamageReasonEnumUtil.toTitleFilter(
          item.damageReason
        )
