  ...
  <div>
    ...
    >
      <div class="max-w">
        ...
        <!-- <p>
          非制单或驳回状态不可删除:&nbsp;&nbsp;<template v-for="(item, index) in arr">{{ index > 0 ? '、' : '' }}{{ item[showField] }}</template>
        </p> -->
        <p>详情:&nbsp;&nbsp;{{ info }}</p>
      </div>
...
export default {
  ...
  created() {
    ...
    if (this.list) {
      ...
      //   if (item.stat === 1 || item.stat === 4) {
      //     this.arr.push(item) 
      //   } else {
      //     this.waitDelete.push(item)
      //   }
      // })
  ...
  methods: {
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          // if (data.corpserialid != null) await damageNoticeServer.delete(data.corpserialid)
          if (data.corpserialno != null) {
            const res = await damageNoticeServer.delete(data.corpserialid)
            if (res.code !== 200) {
