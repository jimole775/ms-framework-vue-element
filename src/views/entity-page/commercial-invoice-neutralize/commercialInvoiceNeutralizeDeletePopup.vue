  ...
  <div>
    ...
    >
      <div class="max-w">
        ...
        <p>
          非制单或驳回状态不能删除:&nbsp;&nbsp;<template v-for="(item, index) in arr">{{ index > 0 ? '、' : '' }}{{ item[showField] }}</template>
        </p>
        <p>详情:&nbsp;&nbsp;{{ info }}</p>
      </div>
...
export default {
  name: 'CommercialInvoiceNeutralizeDeletePopup',
  props: {
    list: {
      type: Array,
      default: null
  ...
  methods: {
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          if (data.invoiceId != null) await commercialInvoiceNeutralizeServer.delete(data.invoiceId)
        } catch (e) {
