...
export default {
  ...
  created() {
    ...
    this.waitDelete = this.waitDelete.filter(item => item.mergeState === 1)
  },
  methods: {
    close() {
      this.$emit('close')
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          if (data.mergeId != null) await creditMergeServer.delete(data.mergeId)
        } catch (e) {
