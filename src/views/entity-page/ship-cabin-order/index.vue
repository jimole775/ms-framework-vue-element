      ...
      <el-row>
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="船名">
            <el-input v-model="keyword.shipName" size="mini" />
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="船次">
            <el-input v-model="keyword.saillings" size="mini" />
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="待订舱">
            <el-switch v-model="keyword.waitForBook" size="mini"/>
          </el-form-item>
        </el-col>
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="船期">
            <el-select v-model="keyword.shipDate" placeholder="请选择" size="mini" class="max-w">
              <el-option
                v-for="item in shipDateList"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <el-col v-show="more" :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="排柜">
            <el-select v-model="keyword.rowCabinetStatus" placeholder="请选择" size="mini" class="max-w">
              <el-option
                v-for="item in rowCabinetStatusList"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          ...
          label="订舱状态"
        >
          <template slot-scope="scope">{{ scope.row.status | ShipCabinOrderStatusFilter }}</template>
        </el-table-column>
        <el-table-column
          v-if="eye.inPortDate"
          show-overflow-tooltip
        ...
        />
        <!-- <el-table-column
          v-if="eye.expectInPortDate"
          show-overflow-tooltip
          prop="expectInPortDate"
          min-width="150"
          sortable
          label="预计到港时间"
        />
        <el-table-column
          v-if="eye.actualInPortDate"
          show-overflow-tooltip
          prop="actualInPortDate"
          min-width="150"
          sortable
          label="实际到港时间"
        /> -->
    ...
    <el-dialog :visible="showEye" width="360px" title="过滤" top="10vh" @close="showEye = false">
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.inPortDate">进港时间</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.cutPortDate">截港时间</el-checkbox>
          </el-col>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.expectInPortDate">预计到港时间</el-checkbox>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.shipName">船名</el-checkbox>
          </el-col>
          <el-col :span="24">
            <el-checkbox v-model="eye.saillings">航次</el-checkbox>
          </el-col>
          <el-col :span="24">
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.reminderType) {
      switch (parseInt(this.$route.query.reminderType)) {
        case 2:
          this.keyword.waitForBook = true
          break
        case 3:
          this.keyword.shipDate = 1
          this.keyword.rowCabinetStatus = 1
          break
        case 4:
          this.keyword.shipDate = 1
          this.keyword.rowCabinetStatus = 2
          break
        case 5:
          this.keyword.shipDate = 2
          this.keyword.rowCabinetStatus = null
          break
        case 16:
    ...
    if (this.$route.query.isMakeBill) {
      this.keyword.isMakeBill = parseInt(this.$route.query.isMakeBill)
    }
  ...
  methods: {
    async search() {
      ...
      const query = {}
      if (this.$route.query && this.$route.query.waitAllotTran) {
        query['waitAllotTran'] = Number.parseInt(this.$route.query.waitAllotTran)
      }
      ...
      query.waitForBook = query.waitForBook ? 1 : 0
    ...
    clickEditBtn(row) {
      ...
      this.$router.push({
        path: './ship-cabin-order/edit/' + row.id
      })
    },
    clickAddBtn() {
      ...
      // this.showEditPopup = true
      this.$router.push({
        path: './ship-cabin-order/edit/new'
      })
    },
    handleRowDb(val) {
      this.$router.push({
        path: './ship-cabin-order/edit/' + val.id
      })
    },
    clickBatchDeleteBtn() {
    ...
    setExportData() {
      ...
      for (const key in initExport) {
        ...
        // this.dataExport[0][key] !== undefined 用于过滤掉数据库没有的字段
        if (this.eye[key] && this.dataExport[0][key] !== undefined) {
