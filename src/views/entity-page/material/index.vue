...
import materialServer from './../../../api/materialServer'
...
export default {
  ...
  components: {
    ...
    MaterialViewPopup: () => import('./materialViewPopup.vue'),
    MaterialEditPopup: () => import('./materialEditPopup.vue'),
    MaterialDeletePopup: () => import('./materialDeletePopup.vue')
  },
  filters: {
  },
  data() {
  ...
  methods: {
    async search() {
      ...
      const res = await materialServer.search({
        query,
        page: this.page - 1,
        size: this.size
      })
