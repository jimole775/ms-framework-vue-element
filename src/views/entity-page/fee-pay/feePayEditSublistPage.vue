...
<script>
import FeePayEditSublist from './../../../components/entity/fee-pay/FeePayEditSublist'
...
export default {
  ...
  methods: {
    save() {
      this.$refs.feePayEdit.save()
    },
    resetData() {
      this.$refs.feePayEdit.resetData()
    },
    emitEvent(event) {
      if (event.key === 'update' && event.params != null && event.params.id != null) {
        this.id = event.params.id
        setTimeout(() => {
          this.$refs.feePayEdit.emitEvent(event)
