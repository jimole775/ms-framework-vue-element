<template>
  <div>
    ...
    <component v-if="showEdit" :is="'CompanyEditPopup'" :id="id" @close="showEdit = false" />
  </div>
</template>
...
export default {
  name: 'CompanyViewPopup',
  components: {
    CompanyEditPopup: () => import('./companyEditPopup.vue')
  },
  filters: {
  },
  props: {
  ...
  methods: {
    ...
    async getData() {
      const res = await companyServer.get(this.id)
