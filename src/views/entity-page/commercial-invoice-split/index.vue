<template>
  <div class="container">
    ...
    <el-button-group>
      <el-button type="primary" icon="el-icon-copy-document" size="mini" @click="gotHistory">历史单据</el-button>
    </el-button-group>
    <el-alert
      :closable="false"
    ...
    <template>
      ...
      >
        ...
        <el-table-column
          ...
          label="贸易类型">
          <template slot-scope="scope">
            {{ scope.row.typeTrade | CommercialInvoiceSplitTradeTypeFilter }}
          </template>
        </el-table-column>
...
export default {
  ...
  components: {
    ...
    CommercialInvoiceViewPopup: () => import('./../commercial-invoice/commercialInvoiceViewPopup.vue'),
    CommercialInvoiceEditPopup: () => import('./../commercial-invoice/commercialInvoiceEditPopup.vue'),
    CommercialInvoiceDeletePopup: () => import('./../commercial-invoice/commercialInvoiceDeletePopup')
  },
  filters: {
  ...
  methods: {
    handleRowDb(val) {
      ...
      this.$router.push({
        path: './commercial-invoice-split/edit/' + code + '/' + this.keyword.mergingSplitType
      })
    },
    ...
    async search() {
      ...
      this.keyword.mergingSplitType = 1
    ...
    async gotHistory() {
      ...
      const urlTo = res.data.data.OMSprod || location.origin
      const url = `${urlTo}fssc_attachservlet?param0=bill_invoice_split_header&param1=${userInfo}`
      window.open(url)
    },
    clickExportBtn() {
      ...
      this.dataExport.forEach(item => {
        ...
        item.typeTrade = CommercialInvoiceSplitTradeTypeEnumUtil.toTitleFilter(item.typeTrade)
