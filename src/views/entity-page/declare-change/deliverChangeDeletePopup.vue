  ...
  methods: {
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          // if (data.corpserialno != null) await deliverReportServer.delete(data.corpserialid)
          if (data.corpserialno != null) {
            const res = await deliverChangeServer.delete(data.corpserialid)
