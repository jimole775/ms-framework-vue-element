<template>
  <div>
    <el-dialog
      ...
      :title="id ? '交单登记-edit' : 'new-交单登记'"
      top="10vh"
      @close="close"
    >
      <div>
        <SubmitOrderEdit ref="submitOrderEdit" :id="id" @loadingChange="loadingChange" />
      </div>
...
export default {
  ...
  methods: {
    ...
    async save() {
      const formData = await this.$refs.submitOrderEdit.save()
