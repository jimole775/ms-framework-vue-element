    ...
    >
      <div class="max-w">
        ...
        <p>
          未删:&nbsp;&nbsp;
          <template
            v-for="(item, index) in waitDelete"
          >{{ index > 0 ? '、' : '' }}{{ item[showField] }}</template>
        </p>
        <p>
          已删:&nbsp;&nbsp;
          <template v-for="(item, index) in deleted">{{ index > 0 ? '、' : '' }}{{ item[showField] }}</template>
        </p>
        <p>详情:&nbsp;&nbsp;{{ info }}</p>
      </div>
      <div slot="footer" class="dialog-footer">
        ...
        <div v-else>
          <el-button
            v-if="deleteing && waitDelete.length > 0"
            size="mini"
            @click="deleteing = false"
          >停止</el-button>
          <el-button
            v-loading="loading"
            v-if="!deleteing && waitDelete.length > 0"
...
export default {
  ...
  created() {
    ...
    this.waitDelete = this.waitDelete.filter(item => item.stat === 1)
  },
  methods: {
    close() {
      this.$emit('close')
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          if (this.list != null) await submitOrderServer.delete(this.list[0].billId)
        } catch (e) {
        ...
        if (this.info === `成功删除 ${data[this.showField]}`) {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
