      ...
      <el-row>
        <el-col :lg="6" :md="8" :sm="12" :xs="24">
          <el-form-item label="截关日期起始">
            <el-date-picker
              v-model="keyword.cut_date_start"
              size="mini"
              class="max-w"
              type="date"
    ...
    <template>
      <el-table
        ...
        <el-table-column
          ...
          label="报关状态">
          <template slot-scope="scope">
            {{ scope.row.customs_status | CustomsBillCustomsStatusFilter }}
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog
      ...
      <div>
        <el-row>
          ...
          <el-col :span="24">
            <el-checkbox v-model="eye.customs_status">报关状态</el-checkbox>
          </el-col>
          <el-col :span="24">
...
export default {
  ...
  components: {
    ...
    CustomsBillFedDeletePopup: () => import('./customsBillFedDeletePopup.vue')
  },
  filters: {
    CustomsBillStatusFilter: CustomsBillStatusEnumUtil.toTitleFilter,
    CustomsBillCustomsStatusFilter: CustomsBillCustomsStatusEnumUtil.toTitleFilter,
  ...
  methods: {
    ...
    async search() {
      ...
      const res = await customsBillFedServer.search({
        keyword,
        page: this.page - 1,
        size: this.size
      })
      this.tableData = res.customsReports
      res.customsReports.forEach((item, index) => {
    ...
    handleRowDb(row) {
      this.$router.push({
        path: './customs-bill-fed/edit/' + row.fk_id
      })
    },
    clickAddBtn() {
      this.$router.push({
        path: './customs-bill-fed/edit/new'
      })
      this.reload()
    },
    clickBatchDeleteBtn() {
    ...
    clickExportBtn() {
      ...
      this.dataExport.forEach(item => {
        ...
        item.customs_status = CustomsBillCustomsStatusEnumUtil.toTitleFilter(item.customs_status)
      })
      this.setExportData()
