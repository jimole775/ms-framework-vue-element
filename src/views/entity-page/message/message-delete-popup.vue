  ...
  <div>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-if="status === 'exception'" @click="close">关闭</el-button>
        <div v-else>
          <el-button v-if="deleteing && waitDelete.length > 0" @click="deleteing = false">停止</el-button>
          <el-button v-loading="loading" v-if="!deleteing && waitDelete.length > 0" type="primary" @click="startDelete">开始</el-button>
          <el-button v-if="!deleteing && waitDelete.length === 0" type="primary" @click="close">完成</el-button>
        </div>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    async startDelete() {
      ...
      for (let i = 0; i < waitDelete.length; i++) {
        ...
        try {
          await messageServer.delete(data.id)
        } catch (e) {
