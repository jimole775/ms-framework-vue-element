...
<script>
  ...
  import advanceNoticeServer from './../../../api/feePayCostInfoServer.js'
  ...
  export default {
    ...
    methods: {
      ...
      async search() {
        ...
        this.keyword.currencycode = this.currencycode
        const keyword = this.keyword
        for (let i = 0; i < this.infos.length; i++) {
          if (this.infos.length === 1 || i === (this.infos.length - 1)) {
            this.keyword.warnId += this.infos[i].warnId
          } else {
            this.keyword.warnId += this.infos[i].warnId + ','
          }
        }
