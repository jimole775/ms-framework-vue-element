      ...
      <el-tab-pane label="基础">
        ...
        >
          <el-row>
            ...
            <el-col :lg="6" :md="8" :sm="12" :xs="24">
              <el-form-item label="应付金额(RMB)" prop="amtRmb">
                <el-input-number v-model="formData.amtRmb" size="mini" />
              </el-form-item>
            </el-col>
            <el-col :lg="6" :md="8" :sm="12" :xs="24">
...
export default {
  ...
  methods: {
    ...
    async getData() {
      this.loading = true
      const res = await feePayInvoiceServer.get(this.id)
    ...
    async save() {
      return new Promise((resolve, reject) => {
        this.$refs['formData'].validate(async valid => {
          if (valid) {
            if (this.formData.id) {
              await feePayInvoiceServer.put(this.formData)
            } else {
              const res = await feePayInvoiceServer.post(this.formData)
