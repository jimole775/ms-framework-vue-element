        ...
        <el-row>
          ...
          <el-col v-if="formData.customs_id" :lg="6" :md="8" :sm="12" :xs="24">
            <el-form-item label="报关单号" prop="customs_code">
              <el-input v-model="formData.customs_code" :disabled="!(customs_code_open_edit === true)" size="mini"/>
            </el-form-item>
          </el-col>
          ...
          <el-col v-if="formData.customs_id" :lg="6" :md="8" :sm="12" :xs="24">
            <el-form-item label="报关日期" prop="customs_date">
              <el-date-picker
                ...
                :disabled="!(customs_date_open_edit === true)"
                size="mini"
                class="max-w"
                type="date"
          ...
          <el-col :lg="6" :md="8" :sm="12" :xs="24">
            <el-form-item label="总体积" prop="total_unit_tj">
              <el-input v-model="total_unit_tj" disabled size="mini"/>
            </el-form-item>
          </el-col>
          <el-col :lg="6" :md="8" :sm="12" :xs="24">
            <el-form-item label="总毛重" prop="total_unit_gw">
              <el-input v-model="total_unit_gw" disabled size="mini"/>
            </el-form-item>
          </el-col>
          <el-col :lg="6" :md="8" :sm="12" :xs="24">
            <el-form-item label="总净重" prop="total_unit_nw">
              <el-input v-model="total_unit_nw" disabled size="mini"/>
            </el-form-item>
          </el-col>
          <el-col :lg="6" :md="8" :sm="12" :xs="24">
      ...
      <el-tabs type="border-card">
        <el-tab-pane label="报关数据">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-input v-model="scope.row.customs_uom" size="mini" @change="unitMonitor"/>
              </template>
            </el-table-column>
            <el-table-column
              show-overflow-tooltip
            ...
            >
              <template slot-scope="scope">
                <el-input v-model="scope.row.second_unit" size="mini" @change="secondUnitMonitor"/>
              </template>
            </el-table-column>
            ...
            <el-table-column show-overflow-tooltip prop="gw" sortable min-width="155" label="单位毛重">
              <template slot-scope="scope">
                ...
                />
                <span v-else>{{ fixedNumber(scope.row.gw)(3) }}</span>
              </template>
            </el-table-column>
            <el-table-column
              show-overflow-tooltip
            ...
            <el-table-column show-overflow-tooltip prop="nw" sortable min-width="155" label="单位净重">
              <template slot-scope="scope">
                ...
                />
                <span v-else>{{ fixedNumber(scope.row.nw)(3) }}</span>
              </template>
            </el-table-column>
            <el-table-column
              show-overflow-tooltip
          ...
          <el-row>
            <el-col :lg="24" :md="24" :sm="24" :xs="24" style="text-overflow: ellipsis;white-space: nowrap;">
              ...
              <span class="spanRight">总毛重：{{ total_unit_gw }}</span>
              <span class="spanRight">总净重：{{ total_unit_nw }}</span>
              <span class="spanRight">总体积：{{ total_unit_tj }}</span>
            </el-col>
          </el-row>
          <el-table
            v-if="showDetailView"
        ...
        <el-tab-pane label="差异金额">
          ...
          >
            ...
            <el-table-column prop="difference_type" label="差异原因" width="300">
              <template slot-scope="scope">
                <CustomsDiffSelect
                  v-model="scope.row.difference_type"
                  @changeData="differenceTypeChange"
                />
              </template>
            </el-table-column>
            <el-table-column prop="difference_amounts" label="差异金额" width="180">
              <template slot-scope="scope">
                <el-input-number
                  v-model="scope.row.difference_amounts"
                  size="mini"
                />
              </template>
            </el-table-column>
...
import CustomsDiffSelect from './CustomsDiffSelect'
...
const emptyFormData = {
  ...
  price_type2: 0,
  prover: null,
  sale_customs_differenceofsale_customs_headers: [], // 差异金额 新增
  saleCustomsDifference: [], // 差异金额 编辑
}
const itemEmptyData = {
  changeable: 1,
  line_type: 4,
...
const boxLineEmptyData = {
  ...
  difference_amounts: null,
  difference_type_descr: null,
  note: null,
}
...
export default {
  ...
  data() {
    return {
      extraPrice: 0,
      customs_code_open_edit: false,
      customs_date_open_edit: false,
  ...
  computed: {
    ...
    total_unit_tj() {
      return this.fixedNumber(this.formData.total_unit_tj)(3)
    },
    total_unit_gw() {
      return this.fixedNumber(this.formData.total_unit_gw)(3)
    },
    total_unit_nw() {
      return this.fixedNumber(this.formData.total_unit_nw)(3)
    },
  },
  watch: {
    id(val) {
  ...
  async created() {
    if (this.id) {
      await this.getData()
      this.isApplyOnlineChange(this.formData.is_apply_online)
    } else {
  ...
  methods: {
    secondUnitMonitor() {      
      this.changeDetailVal()
    },
    unitMonitor(val) {         
      /** 
       * 报关单位为 ‘千克’ 的时候， 报关数量取该行的总净重
       * 财务单价，取报关金额/报关数量
       * */
      if (val && val === '千克') {
        this.items.forEach((item) => {
          if (item.customs_uom === '千克') {
            item.customs_qty = item.unit_nw
            item.customs_price = item.customs_amt / item.customs_qty
          }
        })
        
        let total_qty = 0 // 报关数量
        let total_amt = 0 // 报关金额
        ...
        this.formData.total_qty = total_qty // 报关数量
        this.formData.total_amt = total_amt // 报关金额
        this.formData.total_unit_gw = total_unit_gw // 总毛重
        this.formData.total_unit_nw = total_unit_nw // 总净重
        this.formData.total_unit_tj = total_unit_tj // 总体积
      }
       
      this.changeDetailVal()
    },
    differenceTypeChange(val) {
      this.boxLine.forEach(item => {
        item.difference_type_descr = CustomsBillDiffEnumUtil.toTitle(item.difference_type)
      })
    },
    ...
    updateNameYwInfo(data) {
       ...
       * */
      if (data.uomName === '千克') {
        this.items[this.nameYwIndex].customs_qty = this.items[this.nameYwIndex].unit_nw
        this.items[this.nameYwIndex].customs_price = this.items[this.nameYwIndex].customs_amt / this.items[this.nameYwIndex].customs_qty
      }
    ...
    isApplyOnlineChange(input) {
      if (Number.parseInt(input) === 1) {
        // 当【是否线上报关按钮】为关闭时，【报关单号】及【报关日期】这两个控件 允许用户进行编辑和保存。
        this.customs_code_open_edit = true
        this.customs_date_open_edit = true
      }
      if (Number.parseInt(input) === 0) {
        // 当【是否线上报关按钮】为开启时，【报关单号】及【报关日期】这两个控件 禁用
        this.customs_code_open_edit = false
        this.customs_date_open_edit = false
      }
    },
    ...
    changCprice(index) {
      this.items[index].customs_amt = this.fixedNumber(this.items[index].customs_price * this.items[index].customs_qty)(2)
      this.changeAmt() // 总金额的变化
    },
    changeUnitTj(index) {
      this.items[index].unit_tj = this.fixedNumber(this.items[index].tj * this.items[index].customs_qty)(3)
      this.changeTj() // 总体积的变化
    },
    changeUnitGw(index) {
      this.items[index].unit_gw = this.fixedNumber(this.items[index].gw * this.items[index].customs_qty)(3)
      this.changeGw() // 总毛重的变化
    }, 
    changeUnitNw(index) {
      this.items[index].unit_nw = this.fixedNumber(this.items[index].nw * this.items[index].customs_qty)(3)
    ...
    changeTj() {
      ...
      this.formData.total_unit_tj = tj
      this.changeDetailVal()
    },
    changeGw() {
      let gw = 0
    ...
    changeLineAmt() {
      ...
      this.formData.out_total_amt = amt
      this.changeDetailVal()
    },
    changeQTy() {
      let qty = 0
    ...
    handleCurrentChange(val) {
      if (this.itemDetails.length > 0) {
        ...
        //   // 把数据更新到父表中
        //   this.items[this.selectedHId].sale_customs_lineofsale_customs_h_lines = itemDetails
        // }
      }
      // 切换商品行
      this.itemDetails = []
    ...
    changePrice() {
      ...
      this.items.forEach((item, index) => {
        if (item.new_row !== '2' && item.pro_type !== 4) {
          ...
          //       // if (element.pro_type === 2 || element.pro_type === 3) {
          //         element.customs_price = Number.parseFloat((element.customs_price - partamt / qty).toFixed(2))
          //         element.customs_amt = Number.parseFloat((element.customs_price * element.customs_qty).toFixed(2))
          //         element.print_price = Number.parseFloat((element.customs_price).toFixed(2))
          //         // price += element.customs_price
          //       // }
          //     }
          //   )
          // }
          item.customs_price = Number.parseFloat((item.customs_price - partamt / qty).toFixed(2))
          item.customs_amt = Number.parseFloat((item.customs_price * item.customs_qty).toFixed(2))
          item.print_price = Number.parseFloat((item.customs_price).toFixed(2))
    ...
    recoverPrice() {
      ...
      this.items.forEach((item, index) => {
        ...
        // if (item.pro_type > 0 && item.pro_type < 4 && item.line_type === 1) {
          ...
          item.customs_amt = item.customs_price * item.customs_qty
          // if (item.sale_customs_lineofsale_customs_h_lines) {
          //   item.sale_customs_lineofsale_customs_h_lines.forEach(
          //     (element, i) => {
          //       // if (
          //       //   element.pro_type > 0 &&
          //       //   element.pro_type < 4 &&
          //       //   element.line_type === 1
          //       // ) {
          //         element.customs_price = item.price
          //         element.print_price = item.customs_price
          //         element.customs_amt = item.customs_price * item.customs_qty
          //       }
          //   )
          // }
        // }
    ...
    /** 关联明细行里面的明细行的变更，只要明细行有变更，明细行里的明细行就跟着改变*/
    changeDetailVal() {
      this.items.forEach((item) => {
        if (item.sale_customs_lineofsale_customs_h_lines && item.sale_customs_lineofsale_customs_h_lines.relationData) {
          // 只有一个明细行，就直接赋值
          if (item.sale_customs_lineofsale_customs_h_lines.relationData.length === 1) {
            item.sale_customs_lineofsale_customs_h_lines.relationData.forEach(detailRow => {
              detailRow.customs_qty = item.customs_qty
              detailRow.customs_amt = item.customs_amt
              detailRow.customs_price = item.customs_price
              detailRow.print_price = item.print_price
              detailRow.out_qty = item.qty
              detailRow.second_unit = item.second_unit
              detailRow.customs_uom = item.customs_uom
            })
          } else if (item.sale_customs_lineofsale_customs_h_lines.relationData.length > 1) {
            // 如果有多行明细，按原有比例进行赋值
            let org_print_price = 0
            let org_customs_price = 0
            let org_customs_amt = 0

            // 统计出原始值
            item.sale_customs_lineofsale_customs_h_lines.relationData.forEach(detailRow => {
              org_print_price += detailRow.print_price
              org_customs_amt += detailRow.customs_amt
              org_customs_price += detailRow.customs_price
            })

            // 根据原始值，计算出变更值
            const org_print_price_diff = item.print_price - org_print_price
            const org_customs_price_diff = item.customs_price - org_customs_price
            const org_customs_amt_diff = item.customs_amt - org_customs_amt

            // 赋值逻辑
            item.sale_customs_lineofsale_customs_h_lines.relationData.forEach(detailRow => {
              detailRow.customs_amt += org_customs_amt_diff * (detailRow.customs_amt / item.customs_amt)
              detailRow.customs_price += org_customs_price_diff * (detailRow.customs_price / item.customs_price)
              detailRow.print_price += org_print_price_diff * (detailRow.print_price / item.print_price)
              detailRow.customs_qty = item.customs_qty
              detailRow.out_qty = item.qty
              detailRow.second_unit = item.second_unit
              detailRow.customs_uom = item.customs_uom
    ...
    updateAddDetailInfo(data) {
      ...
      if (data.sale_customs_lineofsale_customs_h_lines && data.sale_customs_lineofsale_customs_h_lines.relationData) {
        data.itemDetails = data.sale_customs_lineofsale_customs_h_lines.relationData
      }
      this.items.push({
        customs_h_id: this.items.length,
        $index: this.items.length,
    ...
    getData() {
      return new Promise(async(s, j) => {
        ...
        try {
          ...
          /**
           * 后端把新增数据和返回的数据用的同一个字段
           * 用boxLine存储之后，把sale_customs_differenceofsale_customs_headers数组置空
           * 否则服务器会把返回的数据当作新增数据
          */
          this.boxLine = [].concat(this.formData.sale_customs_differenceofsale_customs_headers)
          this.formData.sale_customs_differenceofsale_customs_headers = []
    ...
    handleDiffData() {
      ...
      this.boxLine.forEach(item => {
        // 有ID的就是已有数据
        if (item.id) {
          if (!this.formData.saleCustomsDifference) this.formData.saleCustomsDifference = []
          this.formData.saleCustomsDifference.push(item)
        } else {
          // 没有ID的就是新增数据
          this.formData.sale_customs_differenceofsale_customs_headers.push(item)
        }
      })
    },
    del() {
    ...
    clickDeleteBoxLine() {
      if (this.boxLineSelection.length > 0) {
        this.boxLineSelection.forEach(item => {
          for (let i = 0; i < this.boxLine.length; i++) {
            if (JSON.stringify(this.boxLine[i]) === JSON.stringify(item)) {
              // 如果存在ID，就是已有数据，需要把它存入编辑的接口，deleted设置为1
              if (item.id) {
                this.boxLine[i].deleted = 1
                if (!this.formData.saleCustomsDifference) this.formData.saleCustomsDifference = []
                this.formData.saleCustomsDifference.push(this.boxLine[i])
              }
    ...
    shareProcessInfo({ info: { nodeInfo = [] }}) {
      ...
      nodeInfo.forEach((infoItem) => {
        ...
        if (infoItem.status === 'awaiting_check') {
          checkingItem = infoItem
        }
      })

      // 若在财务资料上传节点时，报关单号和报关时间为空则提示用户‘请补充报关单号和报关时间’
      if (Number.parseInt(checkingItem.nodeOrder) >= 3) {
        this.rules.customs_code = [{ required: true, message: '不能为空' }]
        this.rules.customs_date = [{ required: true, message: '不能为空' }]
      }
    ...
    async saveBeforeStart() {
      return new Promise(async(resolve, reject) => {
        this.$refs['formData'].validate(async valid => {
          if (valid) {
            // 财务节点的金额判断
            if (Number.parseInt(this.curProcessNodeInfo.nodeOrder) >= 3) {
              // 拿到额外金额
              this.boxLine.forEach((item) => {
                this.extraPrice += item.difference_amounts
              })
              // 转换精度问题
              this.extraPrice = this.fixedNumber(this.extraPrice)(3)
              this.formData.total_amt = this.fixedNumber(this.formData.total_amt)(3)
              this.formData.out_total_amt = this.fixedNumber(this.formData.out_total_amt)(3)
              // 非分批报:关总金额必须不等于发货总金额加上差异金额，提示
              if (!((this.formData.total_amt - this.extraPrice) === this.formData.out_total_amt) && this.formData.is_repeat_customs === false) {
                this.$message({
                  message: '非分批报关总金额必须等于发货总金额加上差异金额',
                  showClose: true,
                  type: 'warning'
                })
                return reject(new Error('error submit!!'))
              } else {
                // 否则通过
                const items = this.items
                for (let i = 0; i < items.length; i++) {
