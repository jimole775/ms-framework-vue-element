...
function hasPermission(authorities, route) {
  if (route.meta && route.meta.roles) {
    return authorities.some(role => route.meta.roles.includes(role))
  } else {
    return true
  }
}
...
const permission = {
  ...
  mutations: {
    ...
    SET_MEUN: (state, routers) => {
      state.meunRouters = routers
    }
  },
  actions: {
    ...
    GenerateMeun({ commit }, data) {
      commit('SET_MEUN', data)
    }
  }
}
