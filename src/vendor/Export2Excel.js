...
export function export_json_to_excel({
  ...
  let tmpHeader = [];
  if (header.length > 1 && Object.prototype.toString.call(header[0]) === "[object Array]") {
    tmpHeader = [...header]
  }
  else {
  ...
  var wb = new Workbook(),
    ws = sheet_from_array_of_arrays(data, {
      header: tmpHeader // 分开处理表头数据
    });

  if (autoWidth) {
    /*设置worksheet每列的最大宽度*/
    const colWidth = [...tmpHeader, ...data].map(row => row.map(val => {
    ...
    /*以表头的最后一行为初始值*/
    let result = colWidth[tmpHeader.length - 1];
