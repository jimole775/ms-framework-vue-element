...
export default {
  search(params) {
    ...
    if (Validate.isNotEmpty(params.keyword.policyno)) {
      query['policyno'] = params.keyword.policyno
    }
    if (Validate.isNotEmpty(params.keyword.sourceNo)) {
      query['sourceNo'] = params.keyword.sourceNo
    }
    // 查询时增加组织id条件
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiPlnoticeSett/querySdpEdiPlnoticeSett',
      method: 'post',
      data
    }).then(res => {
      res.data.data.data.forEach(item => {
        ...
        item.creditCertificateShift = item.corpopenbankno
  ...
  delete(id) {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiPlnoticeSett/deleteSdpEdiPlnoticeSett',
      method: 'post',
      data
    }).then(res => {
      return res.data
  ...
  add(params) {
    ...
    const data = {
      dataSet: {
        name: 'dataSet',
        config: 'ACT_SDP_EDI_PLNOTICE_SETT_SETT_QUERYBYMAP',
        rows: [
          {
            ...params
          }
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiPlnoticeSett/addSdpEdiPlnoticeSett',
      method: 'post',
      data
    }).then(res => {
      return res.data
  ...
  edit(params) {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiPlnoticeSett/updateSdpEdiPlnoticeSett',
      method: 'post',
      data
    }).then(res => {
      return res.data
