...
export default {
  search(param) {
    ...
    if (Validate.isNotEmpty(param.keyword.corpserialno)) {
      query['corpserialno'] = param.keyword.corpserialno
    }
    if (Validate.isNotEmpty(param.keyword.stat)) {
      query['stat'] = param.keyword.stat
    }
    if (Validate.isNotEmpty(param.keyword.code10)) {
      query['code10'] = param.keyword.code10
    }
    if (Validate.isNotEmpty(param.keyword.buyerno)) {
      query['buyerno'] = param.keyword.buyerno
    }
    if (Validate.isNotEmpty(param.keyword.factInvoiceNo)) {
      query['factInvoiceNo'] = param.keyword.factInvoiceNo
    }
    // 查询时增加组织id条件
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiShipmentapplyinfo/queryEdiShipmentapplyinfo',
      method: 'post',
      data
    }).then(res => {
  ...
  delete(id) {
    ...
    const data = {
      dataSet: {
        name: 'dataSet',
        config: 'ACT_SDP_EDI_SHIPMENTAPPLYINFO_DELETE',
        rows: [{ ...query }],
      }
    }
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiShipmentapplyinfo/deleteEdiShipmentapplyinfo',
      method: 'post',
      data
    }).then(res => {
      return res
  ...
  // /auxhome/sdp/api/1.0.0/sdpEdiShipmentapplyinfo/queryEdiInfo 根据信用证号、买方代码、发票ID导出信息
  queryEdiInfo(data) {
    return request({
  ...
  update(param) {
    const data = {
      dataSet: {
        name: 'dataSet',
        config: 'ACT_SDP_EDI_SHIPMENTAPPLYINFO_UPDATE',
        rows: [{
          ...param
        }]
      }
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiShipmentapplyinfo/updateEdiShipmentapplyinfo',
      method: 'post',
      data
    }).then(res => {
      return res
  ...
  add(param) {
    ...
    const data = {
      dataSet: {
        name: 'dataSet',
        config: 'ACT_SDP_EDI_SHIPMENTAPPLYINFO_ADD',
        rows: [{
          ...param
        }]
      }
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpEdiShipmentapplyinfo/addEdiShipmentapplyinfo',
      method: 'post',
      data
    }).then(res => {
      return res
