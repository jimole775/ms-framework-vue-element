...
export default {
  search(param) {
    ...
    if (Validate.isNotEmpty(param.keyword.salesUserId)) {
      query['salesUserId'] = param.keyword.salesUserId
    }
    if (Validate.isNotEmpty(param.keyword.beginDate)) {
      query['beginDate'] = dateToDateStr(param.keyword.beginDate)
    }
    if (Validate.isNotEmpty(param.keyword.endDate)) {
      query['endDate'] = dateToDateStr(param.keyword.endDate)
    }
    if (Validate.isNotEmpty(param.keyword.isClaiming)) {
      query['isClaiming'] = Number.parseInt(param.keyword.isClaiming)
    }
    if (param.dataItem) {
      query['stat'] = 5
    }
    ...
    return request({
      ...
      res.data.data.data.forEach(item => {
        ...
        item.businessDocuments = item.consultBillNote ? item.consultBillNote.split(',') : [] /* 需业务提供单据... */
  ...
  getCompanyInfo(param) {
    const query = {}
    if (Validate.isNotEmpty(param.keyword.companyCode)) {
      query['companyCode'] = param.keyword.companyCode
    }
    if (Validate.isNotEmpty(param.keyword.countryCode)) {
      query['countryCode'] = param.keyword.countryCode
    }
    if (Validate.isNotEmpty(param.keyword.countryEnName)) {
      query['countryEnName'] = param.keyword.countryEnName
    }
    if (Validate.isNotEmpty(param.keyword.countryName)) {
      query['countryName'] = param.keyword.countryName
    }
    const data = {
      params: { ...query },
      page: {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/baseData/getCompanyInfo',
      method: 'post',
      data
    }).then(res => {
      if (!res.data.data) {
  ...
  get(id) {
    ...
    const data = {
      params: {
        lcBillId: id != null ? parseInt(id, 10) : null
      },
    }
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpFinLcBill/querySdpFinLcBillInfo',
      method: 'post',
      data
    }).then(res => {
      if (!res.data.data) {
  ...
  put(param) {
    ...
    query.sendMan = param.consignorName /* 发货人 */
    ...
    query.receiveMan = param.consigneeName /* 收货人 */
    ...
    query.particularConsigneeName = param.particularConsigneeName /* 具体联系人名称 */
    query.particularConsigneePhone = param.particularConsigneePhone /* 具体联系人电话... */
    query.particularConsigneeFax = param.particularConsigneeFax /* 具体联系人传真... */
    query.particularConsigneeEMail = param.particularConsigneeEMail /* 具体联系人邮箱...*/
    ...
    query.currencyName = param.currencyName /* 货币 */
    ...
    query.fftBank = param.forfaitingBankId /* 福费廷银行... */
    query.lcExchangeBankId = param.confirmingBankId /* 保兑银行 */
    query.lcBillId = param.id /* id */
    query.idpath = param.idpath /* 销售部门的idpath */
    const data = {
      dataSet: {
        name: 'sdpFinLcBill',
        config: 'ACT_SDP_SDPFINLCBILL_BUSNO_001',
        params,
        rows: [query]
      }
    }
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpFinLcBill/updateSdpFinLcBill',
      method: 'post',
      data
    })
  },
  post(param) {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpFinLcBill/addSdpFinLcBill',
      method: 'post',
      data
    })
  },
  delete(id) {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpFinLcBill/deleteSdpFinLcBill',
      method: 'post',
      data
    })
  },
  claim(query) {
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/sdpWorkFlow/turnByAdmin',
      method: 'post',
      data
    }).then(res => {
      return res.data
