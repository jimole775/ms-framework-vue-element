...
export default {
  ...
  // 询价回写oms
    ...
    return request({
      url: '/auxhome/sdp/api/1.0.0/saleEnquiry/updateConfirmShip',
      method: 'post',
      data
    })
  },
  search(param) {
    ...
    return request({
      ...
      res.data.data.dataSet.rows.forEach(item => {
        ...
        /**
         * 处理 { !40QG!: 12 } 这种转义失败的情况
         * 使用 JSON.parse() 会出现语法错误，因为Object的key不能以数字开头
         * */
        const boxNamesNum = {}
        const pureFields = item.boxNamesNum.replace(/[!{}]/g, '').split(',')
        pureFields.forEach((item) => {
          boxNamesNum[`"${item.split(':')[0]}"`] = item.split(':')[1]
        })
        const boxCounts = []
  ...
  get(id) {
    ...
    return request({
      ...
      res.data.data.mustCause = ob.mustCause
       ...
       * */
      const boxNamesNum = {}
      const pureFields = ob.boxNamesNum.replace(/[!{}]/g, '').split(',')
      pureFields.forEach((item) => {
        boxNamesNum[`"${item.split(':')[0]}"`] = item.split(':')[1]
      })
  ...
  put(param) {
    ...
    header['mustCause'] = param.header.mustCause
